...
export function SearchModal({ isOpen, onClose }: Props) {
  ...
  function highlight(text: string, q: string) {
    ...
    return (
      <>
        {text.slice(0, idx)}
        <mark className="bg-blue-500/30 text-blue-600 dark:text-blue-200 rounded px-0.5">{text.slice(idx, idx + q.length)}</mark>
  ...
  function AuditRow({ a, q }: { a: Auditoria; q: string }) {
    return (
      <button
        ...
        className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-100 dark:hover:bg-slate-800/60 transition-colors text-left group rounded-xl"
      >
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <ArrowRight size={12} className="text-slate-400 dark:text-slate-500 group-hover:text-blue-400 transition-colors" />
        </div>
      </button>
    )
  }
  ...
  return (
    <>
      ...
      >
        <div
          className="w-full max-w-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl shadow-black/40 overflow-hidden animate-in slide-in-from-top-4 duration-200"
        ...
        >
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-200 dark:border-slate-800">
            ...
            <input
              ...
              className="flex-1 bg-transparent text-slate-800 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-500 text-sm outline-none"
            />
            {query && (
              <button onClick={() => setQuery('')} className="text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition-colors">
                <X size={14} />
              </button>
            )}
            <kbd className="hidden sm:flex items-center gap-1 px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-[9px] text-slate-400 dark:text-slate-500 font-mono">
              ESC
            </kbd>
          </div>
          ...
          <div className="max-h-[400px] sm:max-h-[480px] overflow-y-auto">
            {showRecent ? (
              ...
              <div className="py-12 text-center">
                <Search size={24} className="text-slate-300 dark:text-slate-700 mx-auto mb-2" />
          ...
          {/* Footer */}
          <div className="px-4 py-2 border-t border-slate-200 dark:border-slate-800 flex items-center gap-4 text-[10px] text-slate-500 dark:text-slate-500">
            <span className="flex items-center gap-1"><kbd className="bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded px-1 font-mono">↵</kbd> abrir</span>
            <span className="flex items-center gap-1"><kbd className="bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded px-1 font-mono">ESC</kbd> fechar</span>
