...
export function SearchModal({ isOpen, onClose }: Props) {
  ...
  function AuditRow({ a, q }: { a: Auditoria; q: string }) {
    return (
      <button
        ...
        className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-800/60 transition-colors text-left group rounded-xl"
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <span className="text-[9px] text-slate-500 flex-shrink-0">{formatDate(a.created_at)}</span>
          </div>
  ...
  return (
    <>
      ...
      >
        <div
          className="w-full max-w-xl bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl shadow-black/60 overflow-hidden animate-in slide-in-from-top-4 duration-200"
        ...
        >
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-800">
            ...
            <kbd className="hidden sm:flex items-center gap-1 px-1.5 py-0.5 bg-slate-800 border border-slate-700 rounded text-[9px] text-slate-500 font-mono">
              ESC
            </kbd>
          </div>
          ...
          <div className="max-h-[400px] sm:max-h-[480px] overflow-y-auto">
            {showRecent ? (
              <div className="p-2">
                <div className="flex items-center gap-1.5 text-[10px] text-slate-500 uppercase tracking-wider font-bold px-2 py-1.5">
                  <Clock size={10} />
                  Auditorias recentes
                </div>
                {recent.map(a => <AuditRow key={a.id} a={a} q="" />)}
                {recent.length === 0 && (
                  <div className="py-8 text-center text-xs text-slate-500">Nenhuma auditoria encontrada</div>
                )}
              </div>
            ) : results.length > 0 ? (
              <div className="p-2">
              ...
              <div className="py-12 text-center">
                ...
                <p className="text-xs text-slate-500">Nenhuma auditoria encontrada para &ldquo;{query}&rdquo;</p>
              </div>
            )}
          </div>
          ...
          <div className="px-4 py-2 border-t border-slate-800 flex items-center gap-4 text-[10px] text-slate-600">
            <span className="flex items-center gap-1"><kbd className="bg-slate-800 border border-slate-700 rounded px-1 font-mono">↵</kbd> abrir</span>
            <span className="flex items-center gap-1"><kbd className="bg-slate-800 border border-slate-700 rounded px-1 font-mono">ESC</kbd> fechar</span>
