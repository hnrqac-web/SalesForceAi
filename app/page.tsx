    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 overflow-x-hidden">
      ...
      <nav className="fixed top-0 inset-x-0 z-50 border-b border-slate-800/50 bg-slate-950/80 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-[10px] text-slate-500 font-medium uppercase tracking-widest border border-slate-700 px-1.5 py-0.5 rounded">Auditor</span>
          </div>
          <div className="flex items-center gap-3">
            <Link href="/login" className="text-sm text-slate-400 hover:text-slate-200 transition-colors font-medium">
              Entrar
            </Link>
            <Link
              href="/login"
              className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-all shadow-lg shadow-blue-600/20"
            >
              Começar grátis
            </Link>
          </div>
      ...
      <section className="pt-32 pb-24 px-6 relative">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <div className="absolute top-20 right-10 w-[200px] h-[200px] bg-violet-600/8 rounded-full blur-3xl" />
        </div>
        ...
        <div className="max-w-4xl mx-auto text-center relative">
          ...
          <h1 className="text-5xl md:text-6xl font-black text-slate-50 leading-tight mb-6 tracking-tight">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              href="/login"
              className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-3.5 rounded-2xl transition-all shadow-xl shadow-blue-600/30 text-sm"
            >
              Acessar o Dashboard
              <ArrowRight size={16} />
            </Link>
            <a
              href="#como-funciona"
              className="flex items-center gap-2 bg-slate-900 hover:bg-slate-800 border border-slate-700 text-slate-300 font-semibold px-8 py-3.5 rounded-2xl transition-all text-sm"
            >
              Como funciona
            </a>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-16">
            {stats.map(s => (
              <div key={s.label} className="bg-slate-900/60 border border-slate-800 rounded-2xl p-4">
                <div className="text-2xl font-black text-blue-400 mb-1">{s.value}</div>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl shadow-black/50">
            ...
            <div className="bg-slate-800/80 px-4 py-3 flex items-center gap-2 border-b border-slate-700">
              ...
              <div className="flex-1 mx-4 bg-slate-700 rounded-lg px-3 py-1 text-[10px] text-slate-400">
            ...
            <div className="p-6">
              <div className="grid grid-cols-4 gap-3 mb-4">
                {[
                  ...
                  { label: 'Leads Críticos', val: '3', color: 'text-red-400' },
                  { label: 'ROI Estimado', val: 'R$21k', color: 'text-emerald-400' },
                ].map(card => (
                  <div key={card.label} className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-3">
                    <div className="text-[9px] text-slate-500 mb-1 uppercase">{card.label}</div>
                    <div className={`text-xl font-black ${card.color}`}>{card.val}</div>
                  </div>
                ))}
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-2 bg-slate-800/60 border border-slate-700/50 rounded-xl p-3 h-28 flex items-end gap-1">
                ...
                <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-3 space-y-2">
                  {['Mariana C.', 'Rafael L.', 'Bruno A.'].map((name, i) => (
                    <div key={name} className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-md bg-blue-600/30 text-blue-400 flex items-center justify-center text-[8px] font-bold">{name[0]}</div>
                      <div className="flex-1 h-1.5 bg-slate-700 rounded-full overflow-hidden">
                        <div className="h-full rounded-full bg-blue-500" style={{ width: `${90 - i * 15}%` }} />
                      </div>
                      <span className="text-[9px] text-blue-400 font-bold">{(9.1 - i * 0.9).toFixed(1)}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section className="py-20 px-6">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl font-black text-slate-50 mb-4">Tudo que seu time precisa</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {features.map(f => (
              <div key={f.title} className={`bg-slate-900 border rounded-2xl p-5 ${f.bg}`}>
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-3 ${f.bg} border`}>
      ...
      <section id="como-funciona" className="py-20 px-6 bg-slate-900/30">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl font-black text-slate-50 mb-4">Configure em 3 passos</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {steps.map((step, i) => (
              <div key={step.num} className="relative">
                {i < steps.length - 1 && (
                  <div className="hidden md:block absolute top-8 left-full w-full h-px bg-gradient-to-r from-blue-500/30 to-transparent -translate-y-1/2 z-0" />
                )}
      ...
      <section className="py-24 px-6">
        <div className="max-w-2xl mx-auto text-center">
          <div className="bg-gradient-to-br from-blue-950/80 to-slate-900 border border-blue-500/20 rounded-3xl p-12 relative overflow-hidden">
            ...
            <div className="relative">
              ...
              <h2 className="text-3xl font-black text-slate-50 mb-4">
