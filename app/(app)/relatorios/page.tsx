    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 md:p-7 space-y-5 overflow-y-auto">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <div className="flex items-center gap-2 text-[10px] text-slate-500 uppercase tracking-wider mb-3 font-bold">
            <Filter size={11} />
            Filtros
          </div>
          <div className="flex flex-wrap gap-3">
            ...
            <select
              ...
              className="bg-slate-800 border border-slate-700 rounded-xl text-slate-300 px-3 py-2 text-xs outline-none focus:border-blue-500 transition-all"
            >
            ...
            <div className="flex items-center gap-2 bg-slate-800 border border-slate-700 rounded-xl px-3 py-2">
              <span className="text-[10px] text-slate-500">Score mín:</span>
              <input
              ...
              />
              <span className="text-xs font-bold text-slate-300 w-6">{scoreMin}</span>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="md:col-span-2 bg-slate-900 border border-slate-800 rounded-xl p-4">
            <div className="text-xs font-semibold text-slate-200 mb-1">Score por Vendedor</div>
            <div className="text-[10px] text-slate-500 mb-3">Média de qualidade no período</div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
            <div className="text-xs font-semibold text-slate-200 mb-1">Sentimentos</div>
            <div className="text-[10px] text-slate-500 mb-3">Distribuição no período</div>
            <div className="h-[180px]">
              ...
              <thead>
                <tr className="bg-slate-950/50 border-b border-slate-800">
                  {['Data', 'Vendedor', 'Cliente', 'Score', 'Sentimento', 'Resumo IA'].map(h => (
                    <th key={h} className="px-4 py-3 text-[10px] text-slate-500 uppercase tracking-widest font-bold">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/50">
                {paginated.length > 0 ? paginated.map(a => (
                  <tr key={a.id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="px-4 py-3 text-[11px] text-slate-500 whitespace-nowrap">{formatDate(a.created_at)}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] text-slate-300">{a.vendedor_name || '—'}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-[11px] text-slate-200 font-medium">{a.cliente_name || '—'}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-[10px] text-slate-500 max-w-[260px] truncate">
                      {a.ai_summary || '—'}
                    </td>
                  </tr>
                )) : (
                  ...
                  return (
                    <button
                      ...
                      className={`w-8 h-8 rounded-lg text-xs font-semibold transition-all ${
                        p === page ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-slate-200 border border-slate-700'
                      }`}
                    >
                      {p}
                    </button>
